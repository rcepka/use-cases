...
# Bring-up all the needed data; note: be careful, it calcullates all the data and takes long time
# source(here("crz", "common", "crz_data_generation.R"))
...
crz_all_contracts_summary_by_years_months <- read_feather(here::here("crz", "data", "crz_all_contracts_summary_by_years_months.feather"))
crz_all_contracts_summary_by_years <- read_feather(here::here("crz", "data", "crz_all_contracts_summary_by_years.feather"))
...
ggplot(
  data = crz_all_contracts_summary_by_years,
  aes(
    x = Date.Year,
    y = N,
...
# Stacked area chart
plot_crz_all_contracts_summary_by_years_months2 <- crz_all_contracts_summary_by_years_months %>%
  mutate(
    Date.Month = month.name[as.numeric(Date.Month)]
    )

ggplot(data = crz_all_contracts_summary_by_years_months) +
  # Area chart
  geom_area(
    aes(
      x = Date.Year,
