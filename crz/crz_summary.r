...
crz_contracts_summary_db <- crz_contracts_full_db %>%
  summarise(
    ContractsAmount = round(sum(contract_price_total_amount, na.rm = TRUE) / 1000000000, digits = 1), # Total amount of all contracts in mld €
    AllContractsQuantity = as.numeric(n()), # Number of all contracts
    AverageContract = round(mean(contract_price_total_amount), digits = 1),# Average contract size
